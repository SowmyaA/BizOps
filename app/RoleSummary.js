(function () {
    ...
    app.controller('RoleSummary', ['$scope', '$http', function ($scope, $http) {

        $scope.division = null;
        $scope.quarterlyData = null;
        $scope.data = null;
        //Get json data
        $http.get("/data/RoleSummary.js")
    ...
    .error(function (data, status, headers, config) {
        ...
        function dashboard(id, fData) {
            var barColor = '#4682b4';
            function segColor(c) { return { IMPLEMENTATIONSPECIALIST: "#807dba", SOLUTIONARCHITECT: "#e08214"}[c]; }

            // compute total for each state.
            fData.forEach(function (d) { d.total = d.freq.IMPLEMENTATIONSPECIALIST + d.freq.SOLUTIONARCHITECT; });
            ...
            hGDim.h = 300 - hGDim.t - hGDim.b;
              ...
              grouped = false;
                    ...
                    .style("fill", function (d, i) {
                        var color = "#807dba";
                        if (i == 0) color = "#807dba"; else if (i == 1) color = "#e08214"; else if (i == 2) color = "#e08214"; else if (i == 3) color = "#800080"; else if (i == 4) color = "#FF8000";
                        return color;
                    });
            ...
            var tF = ['IMPLEMENTATIONSPECIALIST', 'SOLUTIONARCHITECT'].map(function (d) {
        ...
        var freqData = [
    { State: 'AMERICAS', freq: { IMPLEMENTATIONSPECIALIST: 316202, SOLUTIONARCHITECT: 190847} }
    , { State: 'APJ', freq: { IMPLEMENTATIONSPECIALIST: 52606, SOLUTIONARCHITECT: 7680} }
    , { State: 'EMEA', freq: { IMPLEMENTATIONSPECIALIST: 42890, SOLUTIONARCHITECT: 22024} }
    , { State: 'CORP', freq: { IMPLEMENTATIONSPECIALIST: 0, SOLUTIONARCHITECT: 0} }

    ];

        dashboard('#RoleSummary_dashboard', freqData);
        //Download reports
        $scope.download = function () {
            d3.selectAll("svg text").style({ 'font-size': '12px' });
            d3.selectAll(".c3-axis path").style({ 'fill': 'none', 'stroke': '#000' });

            html2canvas(document.getElementById('roleSummaryDIV'), {
